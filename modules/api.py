...
def get_last_price(ticker_symbol: str) -> int:
    '''
    Given a ticker symbol, find current price. If ticker symbol not found,
    ...
    try:
        ...
        data.to_sql('real_time_prices', con=engine,
        ...
        with engine.connect() as connection:
            query_result = connection.execute(db.text("SELECT lastSalePrice FROM \
                                              real_time_prices;")).fetchall()
...
def get_last_price_from_date(ticker_symbol: str, date: str, timeseries: int) -> int:
    '''
    Given a ticker symbol, date, and timeseries, return historical data.
    If ticker symbol not found, return error message.
    '''
    ...
    try:
        ...
        # Send SQLtable from data
        data.to_sql('price_from_date', con=engine,
                    if_exists='replace', index=False)
        ...
        with engine.connect() as connection:
            query_result = connection.execute(db.text("SELECT close FROM \
                                              price_from_date;")).fetchall()
...
def company_lookup(name: str) -> list:
    ...
    try:
        ...
        # Send SQLtable from data
        data.to_sql('company_info', con=engine,
                    if_exists='replace', index=False)
        ...
        with engine.connect() as connection:
            query_result = connection.execute(db.text("SELECT symbol, name \
                                              FROM company_info;")).fetchall()
